...
const calculoCosto = async (TipoItem, Item) =>{
    ...
    if(TipoItem == 'Producto'){
        for(i=0; i< Item.length; i++){
            let producto = await productosDB.findById(Item[i]._idProducto)
...
const utilidades = async  (PrecioTotal, TipoItem, Item, Fecha) =>{
    ...
    if(indicadoresUtilidades){
        ...
        let CantidadTotalBase = +indicadoresUtilidades.CantidadTotal - +CantidadTotal
...
const sucursales = async (Sucursal, PrecioTotal, Fecha) =>{
    let mes = Fecha.split('/')[1]
    let Mes = calculoMes(mes)
    let Anio = Fecha.split('/')[2]
...
indicadoresResta.actualizarIndicadoresResta = async (PrecioTotal, CantidadTotal, ItemProductos, ItemPromociones, ItemMateriales, Sucursal, Cliente, Fecha, Gastos ) =>{
    if(PrecioTotal == null){
        ...
        if(ItemProductos.length > 0){
            TipoItem = 'Producto'
            Item = ItemProductos
            await utilidades(PrecioTotal, TipoItem, Item, Fecha)
        ...
        if(ItemPromociones.length > 0){
            ...
            await promociones(Item, PrecioTotal, CantidadTotal, Fecha, CantidadTotal)
...
module.exports = indicadoresResta